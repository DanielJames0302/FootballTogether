...
export default class Game extends Phaser.Scene {
  ...
  update() {
    ...
    const speed = 200;
    if (!this.room || !this.cursors || !this.ball) { return; }
    if (this.ball_clientID === this.clientID && (Math.abs(this.ball!.body!.velocity.x) > 1 || Math.abs(this.ball!.body!.velocity.y) > 1)) {
